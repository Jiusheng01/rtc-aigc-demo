...
export const useRTC = (): RTCConfig | undefined => {
...
export const useJoin = (): [boolean, () => Promise<void | boolean>] => {
  ...
  const handleAIGCModeStart = async () => {
    dispatch(updateAIReadyState({ isAIReady: false }));
  ...
  async function disPatchJoin(): Promise<boolean | undefined> {
    ...
    try {
      ...
      if (!rtc) {
        throw new Error(`RTC config for scene ${id} is not initialized`);
      }
      RtcClient.setBasicInfo(rtc);

      /** 1. Create RTC Engine */
      await RtcClient.createEngine();
      if (rtc.BusinessId) {
        RtcClient.setBusinessId(rtc.BusinessId);
      }
      ...
      logger.error(`Failed to join call: ${String(error)}`);
      try {
        await RtcClient.stopAgent(id);
      } catch {
        // Start may have failed before an agent became active.
      }
      if (RtcClient.engine) {
        RtcClient.leaveRoom();
      }
