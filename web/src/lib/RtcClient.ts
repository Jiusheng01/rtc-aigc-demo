...
export interface RuntimeRTCConfig {
  AppId: string;
  RoomId: string;
  UserId: string;
  Token: string;
...
export class RTCClient {
  ...
  setBasicInfo = ({ AppId, RoomId, UserId, Token }: RuntimeRTCConfig) => {
    this.basicInfo = {
      app_id: AppId,
      room_id: RoomId,
      user_id: UserId,
  ...
  startAgent = async (scene: string) => {
    ...
    if (this.audioBotEnabled) {
      await this.stopAgent(this.activeSceneId || scene);
    }
    ...
    sessionStorage.setItem('audioBotEnabled', 'true');
  };
  ...
  stopAgent = async (scene: string) => {
    const activeScene = this.activeSceneId || scene;
    if (
      this.sessionId &&
      activeScene &&
      (this.audioBotEnabled || sessionStorage.getItem('audioBotEnabled'))
    ) {
  ...
  stopAgentOnUnload = () => {
    const scene = this.activeSceneId;
    try {
      if (
        this.sessionId &&
        scene &&
        ...
        navigator.sendBeacon(
          `${AIGC_PROXY_HOST}/proxy?Action=StopVoiceChat`,
          new URLSearchParams({ SessionID: this.sessionId, SceneID: scene })
        );
      }
    } finally {
      this.audioBotEnabled = false;
